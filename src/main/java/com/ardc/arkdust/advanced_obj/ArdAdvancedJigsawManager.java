...
@Deprecated
public class ArdAdvancedJigsawManager {
//    private static final Logger LOGGER = LogManager.getLogger();
//
//    public static void addPieces(DynamicRegistries dynamicRegistries, VillageConfig config, JigsawManager.IPieceFactory iPiece, ChunkGenerator chunkGenerator, TemplateManager templateManager, BlockPos pos, List<? super AbstractVillagePiece> pieces, Random random, boolean allowPlaceInside, boolean ignorePosY) {
//        Structure.bootstrap();//#? 进行结构初始处理
//        MutableRegistry<JigsawPattern> mutableregistry = dynamicRegistries.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY);//获取注册器
//        Rotation rotation = Rotation.getRandom(random);//通过随机数获取旋转方向
//        JigsawPattern jigsawpattern = config.startPool().get();//获取拼图模板初始池
//        JigsawPiece jigsawpiece = jigsawpattern.getRandomTemplate(random);//获取随机初始池初始目标
//        AbstractVillagePiece abstractvillagepiece = iPiece.create(templateManager, jigsawpiece, pos, jigsawpiece.getGroundLevelDelta(), rotation, jigsawpiece.getBoundingBox(templateManager, pos, rotation));
...
//        int k;
//        if (ignorePosY) {
//            k = pos.getY() + chunkGenerator.getFirstFreeHeight(i, j, Heightmap.Type.WORLD_SURFACE_WG);
//        } else {
//            k = pos.getY();
//        }
//
...
//        pieces.add(abstractvillagepiece);
...
//            JigsawManager.Assembler jigsawmanager$assembler = new JigsawManager.Assembler(mutableregistry, config.maxDepth(), iPiece, chunkGenerator, templateManager, pieces, random);
//            jigsawmanager$assembler.placing.addLast(new JigsawManager.Entry(abstractvillagepiece, new MutableObject<>(VoxelShapes.join(VoxelShapes.create(axisalignedbb), VoxelShapes.create(AxisAlignedBB.of(mutableboundingbox)), IBooleanFunction.ONLY_FIRST)), k + 80, 0));
//
//            while(!jigsawmanager$assembler.placing.isEmpty()) {
//                JigsawManager.Entry jigsawmanager$entry = jigsawmanager$assembler.placing.removeFirst();
//                jigsawmanager$assembler.tryPlacingChildren(jigsawmanager$entry.piece, jigsawmanager$entry.free, jigsawmanager$entry.boundsTop, jigsawmanager$entry.depth, allowPlaceInside);
//            }
//
//        }
//    }
...
//        jigsawmanager$assembler.placing.addLast(new JigsawManager.Entry(p_242838_1_, new MutableObject<>(VoxelShapes.INFINITY), 0, 0));
//
//        while(!jigsawmanager$assembler.placing.isEmpty()) {
//            JigsawManager.Entry jigsawmanager$entry = jigsawmanager$assembler.placing.removeFirst();
...
//                                                this.placing.addLast(new JigsawManager.Entry(abstractvillagepiece, mutableobject1, l, p_236831_4_ + 1));
//                                            }
//                                            continue label139;
//                                        }
//                                    }
